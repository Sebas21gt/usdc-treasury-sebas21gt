...
export async function rewriteSolanaTs() {
  const code = `
import {
  Connection,
  Keypair,
...
export async function solanaMint(params: {
  ...
  const messageTransmitterProgram = new anchor.Program(MessageTransmitterIdl as anchor.Idl, new PublicKey(CCTP_CONTRACTS.solana.messageTransmitter), provider);
  const tokenMessengerMinterProgram = new anchor.Program(TokenMessengerMinterIdl as anchor.Idl, new PublicKey(CCTP_CONTRACTS.solana.tokenMessenger), provider);
  const usdcMint = new PublicKey(CCTP_CONTRACTS.solana.usdc);
  ...
  const nonce = msgBytes.subarray(12, 20);
  ...
  const findProgramAddress = (label: string, programId: PublicKey, extraSeeds: (string | number[] | Buffer | PublicKey)[] | null = null) => {
    ...
    if (extraSeeds) {
      for (const extraSeed of extraSeeds) {
        ...
        else if (Buffer.isBuffer(extraSeed)) seeds.push(extraSeed);
        else seeds.push(extraSeed.toBuffer());
      }
    }
...
\`;
  fs.writeFileSync('packages/engine/src/chains/solana.ts', code);
}
rewriteSolanaTs();
