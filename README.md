...
Standalone engine process ([`scripts/monitor.ts`](./scripts/monitor.ts)) that reads all three networks' balances every 30s and reports each one's status against its configured min/target/max range. This is separate from the frontend's own balance polling (which is only for the dashboard's display) — it's the read side automatic mode's rebalance logic consumes.

Every completed transfer (manual or automatic) is recorded to `data/history.json` (timestamp, source, destination, amount, burn/mint hashes, mode) and shown in the "Movement History" table on the dashboard, with links to each hash's explorer.
...
Standalone engine process ([`scripts/automatic-mode.ts`](./scripts/automatic-mode.ts)) that checks inventory every 30s and, when a network drops below its configured `min`, rebalances from the network with the largest surplus above its own `target` up to the deficient network's `target` — capped by `AUTOMATIC_MODE_CONFIG.maxAmountPerMove` (30 USDC by default) and a `cooldownMs` (5 minutes by default) between automatic moves. Both live in [`packages/engine/src/config.ts`](./packages/engine/src/config.ts) alongside the per-network min/target/max levels.

**Limitation:** any rebalance that would touch Stellar (either as the surplus source or the deficient destination) is logged and skipped rather than executed. The Stellar treasury wallet is custodial through Pollar and only signs via `signAndSubmitTx` from an authenticated browser session — a headless background process has no such session, so those moves need to be done manually from the dashboard instead. Automatic mode currently only self-executes Polygon ↔ Solana rebalances.
